/*
 core/configuration/serial.hpp - insilico configuration serial
...
void initialize(int argc, char **argv) {
  ...
  std::string external_current_file;
  ...
  usage_error_msg += " -o <output_file.csv> -n <neuron_file.isf> "
      "-s <synapse_file.isf> -e <external_file.isfc>\n\n "
      "Options:\n\t-o   Output file\n\t-n   Neuron configuration"
  ...
  for(unsigned iter = 0; iter < cmds.size(); iter+=2) {
    ...
    switch(cmd.at(1)) {
      ...
      case 'e':
        ...
        break;
      default:
        std::cerr << "Default error: Unknown argument " << cmd << '\n';
        std::cerr << usage_error_msg; exit(0);
    }
  }
  if(repeat[0]) {
    observation_file = output_file;
